package data.missions.cer_EX7Chieftain;
...
public class MissionDefinition implements MissionDefinitionPlugin {
	...
	public void defineMission(MissionDefinitionAPI api) {
		...
		api.setFleetTagline(FleetSide.PLAYER, "Defense Fleet Gamma with Admiral Y-11");
		api.setFleetTagline(FleetSide.ENEMY, "Hegemony fleet under Commodore Jensulte");
		
		// These show up as items in the bulleted list under 
		// "Tactical Objectives" on the mission detail screen
		api.addBriefingItem("The CESN Starbound Must Survive.");
		api.addBriefingItem("Abuse the ShipSystem to Out Tank The enemy Fleet.");
		api.addBriefingItem("Strafe Around your target ship to Hit them in the back.");
		api.addBriefingItem("The CESN Callaban Will Command the Fleet For you, Focus on piloting.");
		
		// Set up the player's fleet.  Variant names come from the
		// files in data/variants and data/variants/fighters
		
				api.addToFleet(FleetSide.PLAYER, "diable_chieftain_EX_Flagship", FleetMemberType.SHIP,"CESN Starbound", true);
		...
		api.defeatOnShipLoss("CESN Starbound");
