package data.missions.cer_EX5Pandemonium;
...
public class MissionDefinition implements MissionDefinitionPlugin {
	...
	public void defineMission(MissionDefinitionAPI api) {
		...
		api.initFleet(FleetSide.PLAYER, "DSF", FleetGoal.ATTACK, true);
		api.initFleet(FleetSide.ENEMY, "HSS", FleetGoal.ATTACK, true);

		// Set a small blurb for each fleet that shows up on the mission detail and
		// mission results screens to identify each side.
		api.setFleetTagline(FleetSide.PLAYER, "Defense Fleet Gamma with Grand Admiral Mordrigg");
		api.setFleetTagline(FleetSide.ENEMY, "Hegemony fleet under Commodore Jensulte");
		
		// These show up as items in the bulleted list under 
		// "Tactical Objectives" on the mission detail screen
		api.addBriefingItem("The CESN Black Pearl Must Survive.");
		api.addBriefingItem("The DSF Star Platinum Will Command the Fleet For you, Focus on piloting.");
		
		// Set up the player's fleet.  Variant names come from the
		// files in data/variants and data/variants/fighters
		
				api.addToFleet(FleetSide.PLAYER, "diable_pandemonium_EX_Flagship", FleetMemberType.SHIP,"CESN Black Pearl", true);
		...
		api.defeatOnShipLoss("CESN Black Pearl");
